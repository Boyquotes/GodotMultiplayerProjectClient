...
func change_outline(grow):
	if !self.has_node("Armature/Skeleton3D/Cube"):
		for child in child_list:
		...
		var child = $Armature/Skeleton3D/Cube
		var material_next_pass : BaseMaterial3D = child.mesh.surface_get_material(0).next_pass
		material_next_pass.grow_amount = grow
